...
class LIFSynapticNeuron(LIFNeuron):
    ...
    def update_spike_times(self):
        ...
        old_spikes_op = self.t_spikes.assign_add(tf.compat.v1.where(self.t_spikes >= 0,
                                                          tf.constant(1.0,
                                                                      shape=[self.max_spikes, self.n_syn]) * self.dt,
                                                          tf.zeros([self.max_spikes, self.n_syn])))

        # Increment last spike index (modulo max_spikes)
        new_idx_op = self.t_spikes_idx.assign(tf.math.floormod(self.t_spikes_idx + 1, self.max_spikes))
        ...
        new_spikes_op = tf.compat.v1.where(self.syn_has_spiked,
