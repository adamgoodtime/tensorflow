...
def check_gradient(f, df, x0, tries=10, deltas=(1e-2, 1e-4, 1)):
...
if __name__ == "__main__":
    ...
    f = lambda x: error_and_BP_gradients(x.reshape((int(np.sqrt(len(x))), int(np.sqrt(len(x))))), return_error=True, quadratic=False)
